...
- [x] Split Claude and Codex hook registrations: Claude keeps `Notification` in `plugin/hooks/hooks.json`; the Codex manifest selects `plugin/hooks/codex-hooks.json`, which contains the supported Codex lifecycle events and uses `PLUGIN_ROOT`.
