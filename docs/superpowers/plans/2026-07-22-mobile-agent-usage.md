...
- [x] Add failing Codex and Claude JSONL fixture assertions.
- [x] Run `bash plugin/tests/test-agent-usage.sh` and confirm the missing parser failure.
- [x] Implement newest-native-usage parsing and context-limit handling.
...
- [x] Add failing route, authorization, and payload assertions.
- [x] Implement the root-scoped token-protected endpoint.
- [x] Confirm state remains transcript-lazy.
...
- [x] Add failing source assertions for lazy fetch and compact formatting.
- [x] Render context, used, and remaining values in the selected chat header.
- [x] Clear stale values immediately when switching sessions.
- [x] Label working app-owned sessions without enabling unsafe interruption.
...
- [x] Run all focused shell tests.
- [x] Run Python compilation and `git diff --check`.
- [x] Restart the dashboard and inspect the selected session in Aside and a 390px viewport.
- [x] Commit the completed change without pushing.
