...
## 뷰어 — 좌우 넘기기 · 채팅에서 바로 열기 (2026-08-03 추가, 커밋 `45d6e19`)

형 요청: "모아보기에서 좌우 넘어가는 기능이 없고, 채팅에서 바로 보거나 눌러서 띄우는 기능이 없다.
이미지 같은거 있으면 접어놓지말고 보여주고."

**계약**: 뷰어가 `openViewer(list, index)` 를 받는다. URL 하나만 받던 시절엔 앞뒤에 뭐가 있는지 몰라 넘길 수가 없었다.

**A안 (형 결정)** — 채팅에서 연 것은 **그 대화에 나온 것만** 대화 순서로 흐른다("해당 채팅에서는 해당 채팅 내용만"). 모아보기에서 연 것은 세션 전체. 목록은 공유 렌더러의 `collectViewables(items)` 가 만든다.

- 같은 파일을 여러 번 고쳤으면 **마지막 등장 자리**로 접는다 — 최신 내용을 보는 게 맞고 위치도 그게 직관적이다.
- 열 수 없는 파일(삭제·초과)은 목록에서 **빼지 않는다** — 조용히 건너뛰면 `n/N` 이 어긋나 혼란스럽다.

**이미지는 접지 않는다.** 활동 그룹과 활동 항목이 둘 다 `<details>` 라 스크린샷·이미지 Read 결과가 두 겹 안에 묻혀 대화를 읽어서는 안 보였다. 이제 그룹이 결과 그림만 접힘 밖으로 끌어올려 스트립으로 내놓고 작업 요약은 그대로 접는다. 항목 쪽 중복 렌더는 제거(펼치면 두 번 나오므로).

**채팅에서 파일 열기.** `file`/`diff` 활동에 대상 경로를 **명시 필드 `item.path`** 로 싣고(추출 규칙은 `_activity_file_path` 하나), 공유 렌더러가 `data-file-path` 로 내보낸다 → 웹·모바일에 동시에 생긴다. `label` 은 표시용이라 계약으로 쓰지 않는다.

**테스트 교훈**: 매달린 참조 검사가 "chat-render → 모바일" **한 방향만** 봤다. 모바일이 `collectViewables` 를 쓰면서 구조분해에 안 넣은 것(브라우저에서만 터짐)을 못 잡아, 역방향 검사를 추가하고 일부러 깨뜨려 잡히는 것까지 확인했다.
