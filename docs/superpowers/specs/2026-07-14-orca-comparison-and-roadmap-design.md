...
- [x] **P0.5 Stale Image Guard**: 마지막 성공 build와 선언된 입력을 비교해 필요한 Start에만
  `--build`를 자동 적용한다.
...
마지막 성공 build의 로컬 baseline을 비교한다. build arg 원문은 로컬 HMAC으로만 기록한다. Compose Watch나
Marina 밖에서 실행한 build를 baseline에 즉시 반영하지는 않아 다음 Start에서 안전한 중복 build가 한 번 생길
수 있다. Clean Rebuild와 Memory Resource Guard는 후속 범위다.
