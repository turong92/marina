...
| 모바일 prompt control | △ | O | 폰에서 기존 agent/terminal에 지시 전송 |
| 모바일 companion full UI | O | X | 비목표 |
...
- [ ] ffmpeg·Node·Chromium 같은 무거운 의존성을 프로젝트별로 분리할지 후속 검증한다.
- [ ] registry cache가 cold-start를 실제로 줄이는지 측정한다. 팀 공유 캐시는 운영 부담이 커 P0에서는 보류한다.
...
- [x] **P0.3 Start / Rebuild / Clean Rebuild 분리**: Start는 기존 이미지, Rebuild는 `up --build`,
  Clean Rebuild는 baseline 제거와 `build --no-cache` 후 기동으로 분리했다.
...
- [x] **P0.5 Stale Image Guard**: 마지막 성공 build의 선언 입력·실제 image ID를 비교해 Start에는 기존
  이미지를 재사용하고, stale이면 명시적 Rebuild 안내를 표시한다.
- [x] **P0.6 Worktree Disk View**: image·BuildKit·volume·`.next` 사용량과 안전한 정리를 제공한다.
- [x] **P0.7 Dockerfile Doctor**: `COPY . . → install`, cache mount 누락, 대형 layer 후보를 경고한다.
- [x] **P0.8 Heavy Dependency Doctor**: Playwright·ffmpeg·torch 같은 무거운 의존성의 상시 설치를 감지한다.
- [x] **P0.9 Memory Resource Guard**: Docker VM·서비스별 메모리와 build pressure를 노출하고 동시 기동
  전에 위험을 경고한다. 실제 Docker 매핑·정책 차단·Aside UI·전체 suite를 검증했다.

  초기 관측값은 host 36 GiB(available 43%), Docker 15.6 GiB, 실행 중 컨테이너 약 10.8 GiB, `web`
  8.93 GiB였다. 이 값은 당시의 실제 사용량이며 BuildKit의 정확한 메모리 할당이 아니다. build peak 표시는
  Docker 일반 컨테이너 사용량과 호스트 여유 메모리에서 얻은 관측 pressure로만 해석한다.

실행 순서는 프로젝트 설정의 dependency cache 경계를 먼저 고친 뒤 marina core 최적화로 넘어간다.
P0.1, P0.3, P0.4, P0.5는 구현·실측했다. 프로젝트가 설정할 Marina 전용 fingerprint
schema는 도입하지 않았다. P0.5는 Compose에 선언된 Dockerfile·`develop.watch` rebuild 입력·build arg와
마지막 성공 build의 로컬 baseline 및 실제 Docker image ID를 비교한다. build arg 원문은 로컬 HMAC으로만
기록한다. Compose Watch나 Marina 밖에서 실행한 build가 image를 교체하면 다음 Start가 image ID 차이를 감지해
명시적 Rebuild 안내를 표시한다. Clean Rebuild는 해당 서비스 baseline을 지우고 `docker compose build --no-cache`
후 기존 Rebuild 경로로 기동한다. P0.6은 카드 메뉴에 worktree 폴더·캐시·build image·Docker 전체
images/BuildKit/volumes 요약을 추가하고, 안전 정리는 marina가 분류한 cache 경로·named volume과 해당
compose build 서비스의 로컬 image ID 삭제로 제한했다. Docker BuildKit prune은 daemon 전역 동작이라 버튼으로
노출하지 않는다. P0.7은 서비스 구성 모달의 Dockerfile Doctor로 `COPY . .` 뒤 dependency install,
dependency install의 BuildKit cache mount 누락, `apt-get install` layer 정리 누락을 읽기 전용으로 경고한다.
P0.8은 같은 Doctor에 Playwright·Chromium·ffmpeg·torch·TensorFlow·OpenCV 같은 무거운 의존성의 상시 설치
감지를 추가했다. 자동 profile 이름, compose 수정, registry cache 적용은 하지 않고 compose-config payload에
진단 결과만 싣는다. Memory Resource Guard는 P0.9에서 완료했다.

### R0. Phone Control

- [x] **R0.1 Mobile Control MVP**: `/mobile` 로그인 화면에서 token을 폰 브라우저에 저장하고, worktree와 기존
  agent/terminal을 골라 작업 지시를 전송한다. `marina mobile token|rotate`와 화면 내 로그아웃으로 접근을 관리한다.
  일반 대시보드 원격 공개와 full 모바일 companion은 하지 않는다.
- [x] **R0.2 Remote Transport Guide**: Tailscale/터널/Caddy 앞단에서 `/mobile`만 노출하는 운영 가이드를 정리하고,
  `marina mobile status`에서 현재 dashboard bind 기준 접근 힌트를 보여준다. 모바일 API는 `/mobile/api/*` 아래에
  두어 원격 프록시가 `/mobile*`만 열면 되게 한다.
- [x] **R0.3a Open-page Notification**: `/mobile` 화면을 열어둔 동안 자동 갱신하고 출력 변경 시 진동/브라우저 알림을
  시도한다.
- [ ] **R0.3 Push Notification**: agent 완료·질문 필요 상태를 모바일 알림으로 전달한다.
