...
- [x] **P0.5 Stale Image Guard**: 마지막 성공 build의 선언 입력·실제 image ID를 비교해 필요한 Start에만
  `--build`를 자동 적용한다.
- [ ] **P0.6 Worktree Disk View**: image·BuildKit·volume·`.next` 사용량과 안전한 정리를 제공한다.
- [ ] **P0.7 Dockerfile Doctor**: `COPY . . → install`, cache mount 누락, 대형 layer 후보를 경고한다.
- [ ] **P0.8 Runtime Profile / Registry Cache**: 선택적 의존성을 나누고 검증된 remote cache 경로를 제공한다.
- [ ] **P0.9 Memory Resource Guard**: Docker VM·서비스별 메모리와 build peak를 노출하고 동시 기동 전에
  위험을 경고한다. 구현은 완료했으며, controller가 실행 중인 dashboard에서 Docker 매핑·정책 차단·Aside
  UI·전체 suite를 검증한 뒤 완료로 표시한다.
...
P0.1, P0.4, P0.5와 P0.3의 Start/Rebuild 분리는 구현·실측했다. 프로젝트가 설정할 Marina 전용 fingerprint
...
build한다. Clean Rebuild와 Memory Resource Guard는 후속 범위다.
