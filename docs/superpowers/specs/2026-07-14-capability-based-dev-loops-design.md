...
- 실행 중인 서비스에 한정한 Compose Watch 프로세스 수명 관리
...
5. 실제 running 상태인 Watch 선언 서비스에만 watcher를 시작한다.

### Rebuild

Start와 같은 prebuild 경로를 거친 뒤 `docker compose up -d --build`를 실행한다. 이후 watcher를 교체한다.
