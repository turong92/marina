...
- Compose Watch나 사용자가 Marina 밖에서 실행한 build의 이벤트·입력 provenance는 추적하지 않는다.
  로컬 image ID drift만 다음 Start에서 감지한다.
...
1. child scan 대기를 500ms로 제한한 snapshot 수집으로 현재 입력을 얻는다.
...
4. 각 호출 전용 immutable overlay를 만든다. 첫 active effective build가 원자적 project Watch lock 아래
   Marina 소유 watcher를 중지하고 build token을 등록한다. 뒤이은 다른 서비스 build는 같은 active generation에
   token만 추가하여 `docker compose up -d --build`를 병렬 실행한다. fast path는 Watch를 중지하지 않는다.
5. build 성공 직후 입력을 다시 수집하고, Compose가 보고한 image ID가 해당 image ref의 실제 ID와 같은지 확인한다.
6. build 전후 입력이 같고 모든 선택 build 서비스의 image ID가 검증된 경우에만 baseline에 merge한다.
7. active build가 남아 있는 동안 shell lifecycle의 Watch refresh는 대기 없이 건너뛴다. 마지막 token이 제거된
   뒤 성공과 실패 모두 Compose Watch를 다시 구성한다. watcher 시작 직전에도 Watch lock 안에서 active state를
   재확인해 config 조회 중 시작된 build와의 TOCTOU를 막는다. 실패, timeout, 입력 drift, image 검증 실패에서는
   baseline을 변경하지 않는다.
...
- snapshot child scan의 대기는 500ms 상한을 유지한다. 임시 파일 준비와 handoff는 best-effort이며 이 상한의
  대상이 아니다.
...
- 각 Compose 호출은 자기 immutable overlay를 사용한다. 동시에 다른 서비스가 시작되어 canonical Watch
  overlay가 갱신되어도 이미 제출된 `up`의 설정은 바뀌지 않는다.
- project Watch lock은 owner PID를 담은 atomic symlink이며 watcher 소유권은 PID start stamp, project 경로,
  command, PGID를 모두 확인한다. lock은 첫 active build가 watcher를 중지하는 짧은 구간에만 잡는다.
- PID-stamped active build token으로 여러 서비스 build가 겹쳐 실행되는 동안 Watch refresh를 억제한다. 마지막
  build가 끝나면 watcher를 복구하며, refresh 조회가 실패하면 직전 watch service 목록을 사용한다.
- fast Start에서 현재 watch service 집합과 resolved Compose model signature가 같고 watcher 소유권이 유효하면
  기존 watcher PID를 유지한다. 서비스가 같아도 Watch 규칙, overlay, build args가 달라지면 watcher를 교체한다.
...
10. inline Dockerfile, preserved-mtime 내용, 디렉터리 권한, 디렉터리 symlink target 변경을 감지한다.
11. build 전후 입력 drift나 Compose image ID/ref 불일치가 baseline을 전진시키지 않는다.
12. stale build 중 Watch가 정지하고 성공·실패 후 복구되며 refresh 실패가 원래 Compose exit code를 바꾸지 않는지 검증한다.
13. 서로 다른 build 서비스는 병렬 실행되고 fast Start는 기존 watcher PID를 유지하는지 검증한다.
14. 기존 Compose dispatch, Build Timeline, Why Rebuilt, Watch 테스트를 함께 실행한다.
