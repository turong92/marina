...
PUBLIC_PATHS = {
    ...
    "/api/auth/claim",
    "/api/auth/login",
    # 앱 설치용 정적 자산 — 로그인 쿠키 없이도 받아져야 한다. 브라우저는 매니페스트와
    # 서비스워커를 **페이지와 별개 요청**으로 가져오고, 그게 401 이면 홈 화면 추가가 그냥
    # 북마크가 되고(주소창 그대로) 서비스워커 등록이 실패해 알림이 원천 불가능해진다.
    # 실측(2026-08-17): 형이 홈 화면에 추가했는데 매니페스트가 401 이라 아무것도 안 걸렸다.
    # 내용에 비밀이 없다 — 코드와 아이콘뿐이고, 알림 내용은 SW 가 인증된 요청으로 따로 가져온다.
    "/sw.js",              # 관할 범위를 / 로 잡으려면 루트에 있어야 한다
    "/mobile/sw.js",
