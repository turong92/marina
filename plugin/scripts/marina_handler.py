...
from marina_mobile import disable_mobile_token, ensure_mobile_token, mobile_access_status, mobile_answer, mobile_catalog, mobile_interrupt, mobile_launch, mobile_clear_uploads, mobile_close_chat, mobile_restart_chat, mobile_forget_chat, mobile_relogin, mobile_remove_room, mobile_rename_room, mobile_request_ok, mobile_set_archived, mobile_set_hidden, mobile_set_pin, mobile_send, mobile_state, mobile_update_session_settings, mobile_upload, mobile_upload_file, render_mobile_html, rotate_mobile_token
    ...
    def do_POST(self) -> None:  # noqa: N802
        try:
            ...
            if parsed.path in ("/mobile/api/pins", "/mobile/api/hidden", "/mobile/api/archive",
                               ...
                               "/mobile/api/clear-uploads", "/mobile/api/close-chat",
                               "/mobile/api/restart-chat",
                ...
                try:
                    ...
                    if parsed.path == "/mobile/api/archive":
                        # 방 접기(스펙 §7). 핀·숨김과 같은 가드를 쓴다 — 남의 워크트리를 접으면 안 된다.
                        root = safe_root(str(mobile_body.get("root", "")))
                        if not self._require_root_access(root):
                            return
                    ...
                    if parsed.path == "/mobile/api/remove-room":
                        ...
                        return
                    if parsed.path == "/mobile/api/close-chat":
                        # 남의 워크트리 프로세스를 끄면 안 된다 — 핀·삭제와 같은 가드.
                        root = safe_root(str(mobile_body.get("root", "")))
                        if not self._require_root_access(root):
                            return
                        self.send_json(mobile_close_chat(mobile_body))
                        return
                    if parsed.path == "/mobile/api/restart-chat":
                        # 끄고 새로 띄우는 한 동작 — close-chat 과 같은 가드다(남의 워크트리
                        # 프로세스를 건드리면 안 된다).
                        root = safe_root(str(mobile_body.get("root", "")))
                        if not self._require_root_access(root):
                            return
                        self.send_json(mobile_restart_chat(mobile_body))
                        return
                    if parsed.path == "/mobile/api/clear-uploads":
                        # 업로드 폴더는 워크트리와 무관한 형의 것 — root 가 필요 없다.
                        self.send_json(mobile_clear_uploads(mobile_body))
                        return
                    ...
                    if parsed.path == "/mobile/api/relogin":
                        ...
                        if not self._require_root_access(root):
                            return
                        self.send_json(mobile_relogin(mobile_body))
                        return
