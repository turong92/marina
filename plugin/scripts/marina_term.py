...
_CHAT_FLAGS = [
    ...
    # 지시문은 **한 줄이면 된다.** 파일이 어디에 놓이든 마리나가 집어오기 때문이다
    # (방 폴더 목록 + SendUserFile 로 건넨 파일). 지시로 동작을 보장하려 들면 문장만 길어지고
    # 모델이 안 들으면 그만이다 — 형: "지시문을 매번 그딴식으로 쳐야돼?".
    # 여기 남긴 건 사람이 읽을 화면을 위한 것뿐이다(절대경로를 늘어놓으면 폰에서 쓸모가 없다).
    "결과물은 파일로 만들어 건네줘. 절대경로를 늘어놓지 말고 파일 이름과 무엇인지만 한 줄로 "
    "말해 — 상대는 폰에서 파일 목록으로 받는다.",
]
