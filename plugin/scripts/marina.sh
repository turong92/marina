...
usage (marina.sh = 내부 launcher; 평소엔 `marina <명령>` 래퍼로 — 래퍼는 `marina start web` 처럼 서비스명을 그대로 받습니다):
  실행 (worktree 단위 — compose 프로젝트는 ~/.marina 의 docker-compose 정의에서 서비스가 옵니다):
    ...
    marina.sh clean-rebuild --<service> | --all # Docker build --no-cache 후 시작
...
# Compose Develop Watch는 foreground 명령이며 프로젝트별 lock 하나를 사용하므로 단일 PID로 supervise한다.
...
main() {
  ...
  shift || true
  case "$command" in
    start|stop|restart|rebuild|clean-rebuild|status|logs|ports)
