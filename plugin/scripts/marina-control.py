...
import marina_access
import marina_remote
import marina_remote_service
import marina_handler

# 전 모듈 심볼을 이 모듈 네임스페이스로 재export — 테스트(spec_from_file_location)가 mc.<symbol> 로 접근.
for _m in (marina_state, marina_dockerfile, marina_logtext, marina_cache, marina_registry, marina_paths, marina_cli, marina_update, marina_compose_svc, marina_sessions, marina_lifecycle, marina_auth, marina_auth_http, marina_access, marina_remote, marina_remote_service, marina_handler):
