...
echo 작업중 > wip.txt && mkdir -p sub && echo 새것 > sub/new.txt   # 미커밋 + untracked
...
assert "wip.txt" in 담긴것 and "sub/new.txt" in 담긴것, 담긴것
...
subprocess.run(["git", "-C", str(repo), "clean", "-qfd"], check=False)
...
대화지우기 = html[html.find("async function forgetChat"):][:900]
