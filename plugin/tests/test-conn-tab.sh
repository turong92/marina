...
grep -q "\.conn-node\b" "$WEB/styles.css" || { echo "FAIL: .conn-node 스타일 없음"; exit 1; }
grep -q "conn-edge-out-host" "$WEB/styles.css" || { echo "FAIL: out(host) 엣지 스타일 없음"; exit 1; }
grep -q "conn-edge-out-svc" "$WEB/styles.css" || { echo "FAIL: out(service) 엣지 스타일 없음"; exit 1; }
