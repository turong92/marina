...
// ① **최근 소식이 위로.** 스펙 §2 는 급한 것부터 올리라고 했지만, 형이 실사용에서 뒤집었다
// (2026-08-23: "진짜 채팅방처럼 가장 최근 메세지 도착한거 순서로"). 방금 답이 온 방이 며칠 전
// "문제" 방 밑에 깔리면 메신저로서 못 쓴다 — 놓치면 안 되는 것은 순서가 아니라 카드의 상태
// 아이콘·라벨로 말한다. 정렬 계약 자체는 test-room-order.sh 가 잠근다.
assert.ok(html.indexOf("슬랙 분석") < html.indexOf("배포 파이프라인"),
  "최근 소식이 위로 오지 않는다");
...
// ⑧-b 모르는 상태도 **라벨은 "쉬는 중"** 이다. 정렬은 이제 상태를 안 보므로(최근 순),
// 여기서 확인할 것은 순서가 아니라 그 어휘다 — 모르는 값이 화면에 그대로 새면 안 된다.
...
assert.ok(섞임.indexOf("모르는것") < 섞임.indexOf("막힌것"), "최근 순이 아니다");
assert.equal(roomStatusLabel("wat"), "쉬는 중", "모르는 상태의 라벨이 사람 말이 아니다");
