...
if bash "$SH" project default "$id" a,zzz >/dev/null 2>&1; then echo "FAIL: accepted non-universe"; exit 1; fi
python3 -c "import json; p=json.load(open('$reg'))['projects'][0]; assert p.get('defaultAttach')==[],p" \
  || { echo "FAIL: rejected write still mutated registry"; exit 1; }

# unknown id → non-zero
if bash "$SH" project default nope a >/dev/null 2>&1; then echo "FAIL: accepted unknown id"; exit 1; fi
