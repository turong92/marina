...
// 10) multiSelect: 체크박스로 그리고, 고른 걸 **여러 개** 표시하고, 즉시 전송하지 않는다.
{
  ...
  // AskUserQuestion 은 내가 준 선택지 뒤에 자유 입력 행을 **항상** 붙인다(tool_input.options 엔 없다).
  // 다중선택에도 있다 — 실물 확인: `4. [ ] Type something`. 다만 넣는 키 순서가 다르다(타이핑 뒤
  // Tab→Enter→Enter, marina_mobile._drive_selector 주석 참조).
