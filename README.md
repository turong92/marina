...
Marina 화면과 API 수준이며, 터미널은 여전히 설치한 OS 사용자 권한으로 실행된다. 관리자는 사용자별 프로젝트 접근과
worktree·terminal·agent 소유자를 설정할 수 있고, Tailscale Serve/Funnel도 `원격 접근`에서 관리한다. 이는 OS 사용자나
파일시스템 권한 분리가 아니므로 신뢰할 수 있는 팀이 같은 개발 호스트를 공유하는 용도다.
