...
service lock으로 직렬화하고 서로 다른 서비스 build는 병렬 실행한다. 첫 active build가 Marina의 Compose
Watch를 잠시 중지하고 마지막 build의 성공·실패 뒤 자동 복구한다. 입력이 같은 fast Start는 watcher도
재시작하지 않는다. build 전후 입력과 실제 image ID가 모두 일치할 때만 baseline을 전진시킨다. 입력 수집에
실패하면 기존 이미지로 시작하면서 Rebuild 안내를 남긴다. build
