...
보낸다. `/mobile` 주소는 그대로 열리며, 처음 한 번 mobile token으로 로그인하면 폰 브라우저에 저장되어 다음부터는
주소만 열면 된다. 기존 Claude/Codex agent 세션 카드는 최근 대화 tail을 보여주고, 답장을 보내면 resume attach로
...
`/mobile`은 로그인 shell을 보여주고, 실제 상태 조회·전송 API(`/mobile/api/state`, `/mobile/api/send`)는 mobile
token을 요구한다. 구버전 호환용 `/api/mobile-state`, `/api/mobile-send`도 남아 있지만, 원격 프록시는 `/mobile*`만
